...
class CMinHash(object):
    ...
    def __init__(
        ...
        c_minhash_rotations: Optional[int] = None,
    ) -> None:
        if c_minhash_rotations is not None: # Hard code
            num_perm = 1
            self.c_minhash_rotations = c_minhash_rotations
            warnings.warn("You provided a CMinHash value (c_minhash_rotations), so num_perm is set to 1.")
        ...
        if c_minhash_rotations is not None:
            self.hashvalues = self._init_hashvalues(1)
    ...
    def update_cminhash_theta_pi(self, b: Iterable) -> None:
        ...
        phv = self.pf(hv)
        phv = np.repeat(phv, self.c_minhash_rotations)

        for i in range(1, self.c_minhash_rotations):
            size = 64 #uint64
            phv_new_head = np.bitwise_and(phv[i-1], 0b1)
            phv[i] = np.bitwise_or(phv[i-1] >> 1, phv_new_head << (size - 1))
    ...
    def rotr(self, num, bits):
        num = np.bitwise_and(num, (1 << bits) - 1)
        bit = np.bitwise_and(num, 1)
        num = numpy.right_shift(num, 1)
        if (bit):
            num = np.bitwise_or(num, 1 << (bits - 1))
        return num
    def update_batch_cminhash_theta_pi(self, b: Iterable) -> None:
        """Update this MinHash with new values.
        The values will be hashed using the hash function specified by
        the `hashfunc` argument in the constructor.
        """
        ...
        for i in range(1, self.c_minhash_rotations) :
            print(phv[i])
            size = len(phv[i])
            phv_new_head = np.bitwise_and(phv[i], 0b1)
            phv[i] = np.bitwise_and(phv[i] >> 1, phv_new_head << (size - 1))
    ...
    def update_batch_cminhash_pi_pi(self, b: Iterable) -> None:
        ...
        hv = np.array([self.pf(_b) for _b in b], dtype=np.uint64, ndmin=2).T

        phv = self.pf(hv)
        phv = np.repeat(phv, self.c_minhash_rotations)

        for i in range(self.c_minhash_rotations):
            self.rotr(phv, i)

        self.hashvalues = np.vstack([phv, self.hashvalues]).min(axis=0)

    def jaccard(self, other: CMinHash) -> float:
    ...
    def merge(self, other: CMinHash) -> None:
    ...
    def __eq__(self, other: CMinHash) -> bool:
        """
        Returns:
    ...
    @classmethod
    def union(cls, *mhs: CMinHash) -> CMinHash:
    ...
    @classmethod
    def bulk(cls, b: Iterable, **minhash_kwargs) -> List[CMinHash]:
    ...
    @classmethod
    def generator(cls, b: Iterable, **minhash_kwargs) -> Generator[CMinHash, None, None]:
